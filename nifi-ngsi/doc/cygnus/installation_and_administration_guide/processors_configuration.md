...
The NGSIToPostgreSQL processor takes the FLowFile generated by the ListenHTTP processor and 
transform it into a NGSI event, after this processor is communication with the controller service (DBCPConnectionPoll) that was previously set 
in the processor. The processor will create the database, the tables and insert the data into the PostgreSQL database using the structure defined by the NGSI standard.
...
The NGSIToMongo processor takes the FLowFile generated by the ListenHTTP processor and 
transform it into a NGSI event, after this processor will create the Schema, the cocuments and collections insert 
the data into the MongoDB using the structure defined by the NGSI standard.
